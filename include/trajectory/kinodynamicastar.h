...
public:
  ...
  /* main API */
  void setParam(ros::NodeHandle& nh,MapSearchSpace::Ptr& map_i);
  void init();
  void reset();
  ...
  std::vector<Eigen::Vector3d,Eigen::aligned_allocator<Eigen::Vector3d>> getKinoTraj(double delta_t);
  ...
  float getExucuteTime(){
      double t = 0;
      if (is_shot_succ_) t+= t_shot_;
